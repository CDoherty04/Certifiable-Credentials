async function handleFormSubmission(e) {
    ...
    try {
        ...
        const { statusDiv, submitButton } = elements;

        // Now validate the form data
        const validationResult = validateFormData(elements);
        if (!validationResult.isValid) {
            // Display validation error
            statusDiv.style.display = 'block';
            statusDiv.className = 'status error';
            statusDiv.textContent = `Error: ${validationResult.errorMessage}`;
            return false;
        }

        const { fullFormData } = validationResult;
        ...
        try {
            ...
            if (error.message.includes('Credential data field cannot be empty')) {
                errorMessage = 'Please enter credential data in the JSON field';
            } else if (error.message.includes('Issuer seed field cannot be empty')) {
                errorMessage = 'Please enter your issuer seed';
            } else if (error.message.includes('Subject address field cannot be empty')) {
                errorMessage = 'Please enter the subject address';
            } else if (error.message.includes('Subject email field cannot be empty')) {
                errorMessage = 'Please enter the subject email';
            } else if (error.message.includes('Invalid JSON format')) {
                errorMessage = 'Please check that your credential data is valid JSON format';
            } else if (error.message.includes('Credential data must contain at least one field')) {
                errorMessage = 'Please add at least one field to your credential data';
        ...
        // This catch block handles errors from getElements() or validateFormData()
        console.error('Error in form submission:', error);
        // We can't display the error here because we don't have access to statusDiv yet
        return false;
    }
}

function validateFormData(elements) {
    try {
        const { credentialDataElement, issuerSeedElement, subjectAddressElement, subjectEmailElement, imageUploadElement } = elements;
        
        const credentialDataValue = credentialDataElement.value.trim();
        const issuerSeedValue = issuerSeedElement.value.trim();
        const subjectAddressValue = subjectAddressElement.value.trim();
        const subjectEmailValue = subjectEmailElement.value.trim();

        // Validate that all required fields have values
        if (!credentialDataValue) {
            return { isValid: false, errorMessage: 'Please enter credential data in the JSON field' };
        }

        if (!issuerSeedValue) {
            return { isValid: false, errorMessage: 'Please enter your issuer seed' };
        }

        if (!subjectAddressValue) {
            return { isValid: false, errorMessage: 'Please enter the subject address' };
        }

        if (!subjectEmailValue) {
            return { isValid: false, errorMessage: 'Please enter the subject email' };
        }

        // Validate JSON format
        let parsedCredentialData;
        try {
            parsedCredentialData = JSON.parse(credentialDataValue);
        } catch (parseError) {
            return { isValid: false, errorMessage: 'Please check that your credential data is valid JSON format' };
        }
        
                const image = imageUploadElement.files[0];
                if (!image) {
                    return { isValid: false, errorMessage: 'Please select an image file to upload' };
                }

        // Check if parsed credential data is empty object or has no meaningful content
        if (!parsedCredentialData || Object.keys(parsedCredentialData).length === 0) {
            return { isValid: false, errorMessage: 'Please add at least one field to your credential data' };
        }

        // If all validation passes, create the form data
        const formData = {
            issuerSeed: issuerSeedValue,
            subjectAddress: subjectAddressValue,
            subjectEmail: subjectEmailValue,
        ...
        return { isValid: true, fullFormData };
    } catch (error) {
...
let selectedImage = null;
    ...
    function getElements() {
        ...
        // Return all the DOM elements for validation later
        return { 
            credentialDataElement, 
            issuerSeedElement, 
            subjectAddressElement, 
            subjectEmailElement, 
            imageUploadElement, 
            statusDiv, 
            submitButton 
